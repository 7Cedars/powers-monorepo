...
```mermaid
graph TD
    A[Executive Proposes Action] -->|Wait 5 Days| B{Vetoed by Members?}
    B -- Yes --> C[Action Blocked]
    B -- No --> D[Executive Executes Action]
    D --> E[Registry Updated]
```
...
```mermaid
graph TD
    A[Member Self-Nominates] --> B[Election Started]
    B --> C[Members Vote]
    C --> D[Tally & Assign Roles]
    D --> E[Cleanup]
```
