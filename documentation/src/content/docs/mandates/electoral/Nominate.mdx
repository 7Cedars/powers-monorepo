...
This mandate provides a simple interface for users to signal their intent to run for a role or position. It interacts with a helper `Nominees` contract which maintains the list of active candidates. This list is often used by other mandates (like `DelegateTokenSelect` or `ElectionRegistry_Vote`) to conduct elections.
