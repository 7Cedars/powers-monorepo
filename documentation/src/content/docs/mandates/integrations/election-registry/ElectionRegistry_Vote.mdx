---
title: "ElectionRegistry_Vote"
description: "Documentation for the ElectionRegistry_Vote integration mandate, which allows users to cast votes in a specific election instance."
---

ElectionRegistry_Vote is an integration mandate that allows users to cast votes in a specific election instance.

## Overview

This mandate is typically deployed dynamically (via `ElectionRegistry_CreateVoteMandate`) for each new election. It serves as the interface for casting votes. By encapsulating the voting logic in a separate mandate, the system can enforce specific access controls (e.g., only `voterRoleId` can call it) and parameters (e.g., `maxVotes`) unique to that election event.

## Configuration

When adopting an ElectionRegistry_Vote instance, the following parameters are required:

1.  `ElectionRegistryContract` (address): The address of the ElectionRegistry helper contract.
...
2.  **Cast Vote**
    *   Calls `vote(electionId, caller, vote)` on the `ElectionRegistry` contract.
    *   The `ElectionRegistry` contract handles the recording of votes and prevention of double-voting.
