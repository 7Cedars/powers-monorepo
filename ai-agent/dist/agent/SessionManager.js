...
import { createSession, sessionToSummary, zeroSessionKeys, } from './AgentSession.js';
class SessionManager {
    sessions = new Map();
    async createSession(input, onDestroy) {
        const ttlMs = Math.max(config.session.ttlMinMs, Math.min(config.session.ttlMaxMs, input.ttlMs));
        ...
        console.log(`[session] created ${sessionId} for ${session.userAddress} (TTL ${ttlMs}ms)`);
        return sessionId;
    }
    destroySession(sessionId, onDestroy) {
    ...
    findSessionsForGroup(groupName) {
        ...
        for (const session of this.sessions.values()) {
            if (!session.xmtpClient)
                continue;
            const hasOrg = session.organisations.some((o) => o.chainId === chainId &&
    ...
    touchSession(sessionId, onDestroy) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
    ...
    addOrganisation(sessionId, org) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return false;
        const exists = session.organisations.some((o) => o.powersAddress === org.powersAddress && o.chainId === org.chainId);
        if (!exists)
            session.organisations.push(org);
        return true;
    }
}
