...
const Nominate = {
    ...
    encode: (p) => encodeAbiParameters(parseAbiParameters('bool'), [Boolean(p.shouldNominate)]),
};
const PeerSelect = {
    name: 'PeerSelect',
    category: 'electoral',
    ...
    encode: (p) => encodeAbiParameters(parseAbiParameters('bool[]'), [
        p.selections,
    ]),
};
const RenounceRole = {
    ...
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('uint256'), [BigInt(p.roleId)]),
};
const AssignExternalRole = {
    name: 'AssignExternalRole',
    category: 'electoral',
    description: 'Syncs an account role between this Powers contract and an external Powers contract.',
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('address'), [p.account]),
};
const RoleByRoles = {
    name: 'RoleByRoles',
    category: 'electoral',
...
const OpenAction = {
    ...
    encode: (p) => encodeAbiParameters(parseAbiParameters('address[], uint256[], bytes[]'), [
        p.targets,
        p.values.map((v) => BigInt(v)),
        p.calldatas,
    ]),
};
const PresetActions = {
...
const PresetActions_OnOwnPowers = {
...
const StatementOfIntent = {
    ...
    encode: (p) => {
        ...
        if (args.length === 0)
            return '0x';
        return encodeAbiParameters(args.map(() => ({ type: 'string' })), args);
    },
};
const BespokeAction_Simple = {
    ...
    encode: (p) => p.rawCalldata ?? '0x',
};
const BespokeAction_Advanced = {
...
const BespokeAction_OnReturnValue = {
...
const CheckExternalActionState = {
...
const ExternalAction_OnReturnValue = {
    ...
    encode: (p) => encodeAbiParameters([{ type: 'address' }, { type: 'uint16' }], [p.powersTarget, Number(p.mandateIdTarget)]),
};
// ── Reform ───────────────────────────────────────────────────────────────────
const Adopt_Mandates = {
    name: 'Adopt_Mandates',
    category: 'reform',
    description: 'Proposes adoption of one or more new mandate contracts with allowed role IDs.',
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('address[], uint256[]'), [
        p.mandates,
        p.roleIds.map((r) => BigInt(r)),
    ]),
};
const Revoke_Mandates = {
    name: 'Revoke_Mandates',
    category: 'reform',
    description: 'Revokes one or more adopted mandates by ID.',
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters([{ type: 'uint16[]' }], [p.mandateIds.map(Number)]),
};
const PauseMandates = {
    name: 'PauseMandates',
    category: 'reform',
    ...
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('bool'), [Boolean(p.paused)]),
};
const MandatePackage = {
    name: 'MandatePackage',
    category: 'reform',
...
const ElectionRegistry_Nominate = {
    name: 'ElectionRegistry_Nominate',
    category: 'integration',
    description: 'Nominates or revokes nomination for an election by title.',
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('string'), [p.title]),
};
const ElectionRegistry_CreateVoteMandate = {
...
const ElectionRegistry_CleanUpVoteMandate = {
...
const SlateRegistry_AddSlate = {
    name: 'SlateRegistry_AddSlate',
    category: 'integration',
    description: 'Proposes a named slate of actions for a running slate-based election.',
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('string, string, address[], uint256[], bytes[]'), [
        p.electionTitle,
        p.slateDescription,
        p.targets,
        p.values.map(BigInt),
...
const SlateRegistry_RemoveSlate = {
...
const SlateRegistry_ExecuteResult = {
    ...
    encode: (p) => encodeAbiParameters(parseAbiParameters('string'), [p.electionTitle]),
};
// ── Integrations — Safe ───────────────────────────────────────────────────────
const Safe_ExecTransaction = {
...
const Safe_ExecTransaction_OnReturnValue = {
...
const GovernedToken_MintEncodedToken = {
    ...
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('address'), [p.to]),
};
// ── Integrations — Governor ───────────────────────────────────────────────────
const Governor_CreateProposal = {
    name: 'Governor_CreateProposal',
    category: 'integration',
    description: 'Creates a proposal on an OZ Governor contract.',
    calldataShape: 'structured',
    encode: (p) => encodeAbiParameters(parseAbiParameters('address[], uint256[], bytes[], string'), [
        p.targets,
        p.values.map(BigInt),
        p.calldatas,
        p.description,
...
const Governor_ExecuteProposal = {
...
const PowersFactory_AddSafeDelegate = {
...
const PowersFactory_AssignRole = {
...
const byName = new Map(ALL_TEMPLATES.map((t) => [t.name.toLowerCase(), t]));
...
export function findTemplate(nameDescription) {
...
export function listTemplates() {
    return ALL_TEMPLATES.map((t) => `${t.name} (${t.category}) — ${t.description}`).join('\n');
}
