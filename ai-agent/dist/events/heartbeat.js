...
export function startHeartbeat(session, org) {
    const key = `${org.chainId}:${org.powersAddress}`;
    const timer = setInterval(() => runHeartbeat(session, org).catch((err) => console.error(`[heartbeat] error (session=${session.sessionId} org=${key}):`, err)), HEARTBEAT_INTERVAL_MS);
    session.heartbeatTimers.set(key, timer);
    console.log(`[heartbeat] started for session=${session.sessionId} org=${key}`);
}
export function stopHeartbeat(session, org) {
    const key = `${org.chainId}:${org.powersAddress}`;
...
async function runHeartbeat(session, org) {
    const key = `${org.chainId}:${org.powersAddress}`;
    ...
    const groupReply = homeGroup
        ? async (text) => { await homeGroup.sendText(text); }
        : async (_text) => { };
    await reason(session, org, conversationId, HEARTBEAT_TRIGGER, groupReply, 'heartbeat');
}
async function resolveHomeGroup(session, org) {
    ...
    try {
        ...
        if (agentRoles.length === 0)
            return null;
        const activeMandates = mandates.filter((m) => m.active && agentRoles.includes(m.conditions.allowedRole));
        if (activeMandates.length === 0)
            return null;
        // Prefer admin role (roleId 0) group first
        const adminMandate = activeMandates.find((m) => m.conditions.allowedRole === 0n);
