...
export function startWatchers(session, org) {
    const unwatchers = [];
    ...
    const unwatchRoleSet = getWatchClient(org.chainId).watchContractEvent({
        ...
        onLogs: async (logs) => {
            for (const log of logs) {
                const { account, roleId, access } = log.args;
                if (account.toLowerCase() === session.userAddress.toLowerCase()) {
                    if (access) {
                        console.log(`[watcher] ${session.sessionId} gained role ${roleId} on ${org.powersAddress}`);
                        await handleRoleGained(session, org, roleId);
                    }
                    else {
                        console.log(`[watcher] ${session.sessionId} lost role ${roleId} on ${org.powersAddress}`);
                    }
                }
                else if (access) {
    ...
    const unwatchProposed = getWatchClient(org.chainId).watchContractEvent({
        ...
        onLogs: async (logs) => {
            for (const log of logs) {
                const { actionId, mandateId } = log.args;
                // Don't react to proposals we made ourselves
                const caller = log.args.caller;
                if (caller?.toLowerCase() === session.userAddress.toLowerCase())
                    continue;
                console.log(`[watcher] ProposedActionCreated action=${actionId} mandate=${mandateId} (${orgKey})`);
                updateLastEventTime(session, orgKey);
                await triggerReasonOnMandateGroup(session, org, mandateId, `[ON-CHAIN EVENT] A new action has been proposed.\nActionId: ${actionId}\nMandateId: ${mandateId}\n\nReview the governance state and decide whether to vote.`);
            }
        },
        onError: (err) => {
    ...
    const unwatchVote = getWatchClient(org.chainId).watchContractEvent({
        ...
        onLogs: async (logs) => {
            for (const log of logs) {
                ...
                try {
                    const { getActionData, getActionVoteData, getCurrentBlock, getAllMandates } = await import('../powers/contract.js');
                    const [actionData, voteData, currentBlock, mandates] = await Promise.all([
                        getActionData(org.chainId, org.powersAddress, actionId),
                    ...
                    if (currentBlock < BigInt(voteData.voteEnd) + BigInt(timelock))
                        continue;
                    console.log(`[watcher] action ${actionId} is Succeeded + timelock cleared (${orgKey})`);
                    updateLastEventTime(session, orgKey);
                    await triggerReasonOnMandateGroup(session, org, actionData.mandateId, `[ON-CHAIN EVENT] Action ${actionId} has Succeeded and the timelock has cleared.\nMandateId: ${actionData.mandateId}\n\nReview and execute if appropriate.`);
                }
                catch (err) {
...
async function handleRoleGained(session, org, roleId) {
    ...
    try {
        ...
        const mandatesForRole = mandates.filter((m) => m.active && m.conditions.allowedRole === roleId);
        for (const mandate of mandatesForRole) {
            ...
            if (!group)
                continue;
            const introTrigger = `[ON-CHAIN EVENT] You have just been granted role ${roleId}, ` +
            ...
            await reason(session, org, group.id, introTrigger, async (text) => { await group.sendText(text); }, 'on_chain_event');
        }
    }
    catch (err) {
...
async function handleMemberAdded(session, org, newMember) {
    const memberIsPowers = await isPowersContract(org.chainId, newMember);
    if (!memberIsPowers)
        return;
    console.log(`[watcher] new member ${newMember} is a Powers instance — refreshing linked cache for ${org.powersAddress}`);
    try {
        const mandates = await getAllMandates(org.chainId, org.powersAddress);
...
async function triggerReasonOnMandateGroup(session, org, mandateId, triggerText) {
    if (!session.xmtpClient)
        return;
    const groupName = getGroupName('Mandate', org.chainId, org.powersAddress, BigInt(mandateId));
    const group = await findGroup(session, groupName);
    if (group) {
        await reason(session, org, group.id, triggerText, async (text) => { await group.sendText(text); }, 'on_chain_event');
    }
    else {
        // Agent has no group to chat in, but can still reason and act on-chain
        console.log(`[watcher] no group found for mandate ${mandateId} — reasoning silently`);
        await reason(session, org, `silent:${org.powersAddress}:${mandateId}`, triggerText, async () => { }, 'on_chain_event');
    }
}
function updateLastEventTime(session, orgKey) {
