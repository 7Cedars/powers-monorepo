...
export function createServer(onSessionStart, onSessionDestroy, onOrgAdded) {
    const app = express();
    ...
    app.post('/api/session/start', async (req, res) => {
        try {
            ...
            const sessionId = await sessionManager.createSession(input, (session) => onSessionDestroy(session.sessionId));
            onSessionStart(sessionId).catch((err) => console.error(`[api] session start error for ${sessionId}:`, err));
    ...
    app.patch('/api/session/:sessionId/persona', (req, res) => {
    ...
    app.post('/api/session/:sessionId/organisations', async (req, res) => {
        const session = sessionManager.getSession(req.params.sessionId);
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
    ...
    app.get('/api/session/:sessionId/skills', (req, res) => {
        const session = sessionManager.getSession(req.params.sessionId);
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
    ...
    app.delete('/api/session/:sessionId/skills/:skillName', (req, res) => {
        const session = sessionManager.getSession(req.params.sessionId);
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
    ...
    app.get('/api/session/:sessionId/fund', async (req, res) => {
        const session = sessionManager.getSession(req.params.sessionId);
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
