...
function getClients(session, org) {
...
export async function propose(session, org, mandateId, mandateCalldata, nonce, uri) {
    ...
    const actionId = logs[0]?.args?.actionId;
    if (actionId === undefined) {
        throw new Error('ProposedActionCreated event not found in receipt');
    }
    return { actionId, txHash };
}
export async function castVoteWithReason(session, org, actionId, support, reason) {
    const { walletClient, publicClient } = getClients(session, org);
    await publicClient.simulateContract({
        address: org.powersAddress,
        abi: powersAbi,
...
export async function request(session, org, mandateId, mandateCalldata, nonce, uri) {
    const { walletClient, publicClient } = getClients(session, org);
    await publicClient.simulateContract({
        address: org.powersAddress,
        abi: powersAbi,
