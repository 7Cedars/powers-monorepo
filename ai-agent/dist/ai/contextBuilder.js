import { getAllMandates, getOpenActions, getAgentRoles, getCurrentBlock, getEthBalance, getOrgName, getOrgUri, getAllRoleInfo, actionStateLabel, } from '../powers/contract.js';
import { formatLinkedInstancesSummary } from '../powers/linkedInstances.js';
export async function buildContext(session, org, triggeredBy = 'xmtp_message', groupName = '', groupType = 'unknown', contextId = 0) {
    const [mandates, openActions, agentRoles, currentBlock, agentEthBalance, orgName, orgUri] = await Promise.all([
...
export function formatContextMessage(ctx) {
    const ethBalanceFormatted = (Number(ctx.agentEthBalance) / 1e18).toFixed(6);
