...
import { governanceToolDefinitions, handleToolCall, } from './tools/governanceTools.js';
import { config } from '../config/env.js';
import { sessionManager } from '../agent/SessionManager.js';
import { claudeLimiter } from './rateLimiter.js';
export async function reason(session, org, conversationId, triggerText, groupReply, triggeredBy = 'xmtp_message') {
    ...
    const history = session.histories.get(conversationId) ?? [];
    ...
    const apiParams = thinkingBudget > 0
        ? { thinking: { type: 'enabled', budget_tokens: thinkingBudget }, max_tokens: Math.max(4096, thinkingBudget + 2000) }
        : { max_tokens: 4096 };
    ...
    while (rounds < maxRounds) {
        ...
        let response;
        try {
            response = await claudeLimiter.schedule(() => session.claudeClient.messages.create({
        ...
        if (response.stop_reason === 'end_turn' || response.stop_reason === 'stop_sequence') {
            ...
            for (const block of response.content) {
                if (block.type === 'text' && block.text.trim()) {
                    ...
                    if (triggeredBy === 'heartbeat' && block.text.trim().length === 0)
                        continue;
        ...
        for (const block of response.content) {
            ...
            console.log(`[reason] tool_use: ${toolBlock.name} (session ${session.sessionId})`);
            let result;
            try {
                result = await handleToolCall(toolBlock.name, toolBlock.input, toolCtx);
            }
            catch (err) {
    ...
    const sentAny = assistantParts.some((m) => Array.isArray(m.content) &&
...
async function sendRateLimited(session, conversationId, text, groupReply) {
