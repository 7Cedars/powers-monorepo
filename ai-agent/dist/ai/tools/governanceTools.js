...
import { getActionData, getActionUri, hasVoted, ActionState, } from '../../powers/contract.js';
import { propose, castVoteWithReason, request, } from '../../write/powersWriter.js';
...
export const governanceToolDefinitions = [
    {
        ...
        description: 'Fetch the current on-chain governance state for the active organisation. Call this before any governance action to ensure you have fresh data.',
        input_schema: {
            type: 'object',
            properties: {},
            required: [],
    ...
    {
        name: 'propose_action',
        description: 'Propose a new governance action on a mandate. Encodes calldata from the mandate template automatically when possible.',
        input_schema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'execute_action',
        description: 'Execute a passed governance action once the timelock has cleared. Verifies state before submitting.',
        input_schema: {
            type: 'object',
            properties: {
                actionId: { type: 'string', description: 'Decimal action ID.' },
...
function resolveOrg(session, defaultOrg, powersAddress) {
    if (!powersAddress)
        return defaultOrg;
    const found = session.organisations.find((o) => o.powersAddress.toLowerCase() === powersAddress.toLowerCase());
    return found ?? defaultOrg;
}
export async function handleToolCall(toolName, toolInput, ctx) {
    ...
    switch (toolName) {
        ...
        case 'fund_check': {
            ...
            const lines = await Promise.all(session.organisations.map(async (o) => {
        ...
        case 'propose_action': {
            const targetOrg = resolveOrg(session, org, toolInput.powersAddress);
            const mandateId = Number(toolInput.mandateId);
            const nonce = toolInput.nonce ? BigInt(toolInput.nonce) : BigInt(Date.now());
            const uri = String(toolInput.uri ?? '');
            const reasoning = String(toolInput.reasoning ?? '');
            let calldata;
            if (toolInput.mandateCalldata) {
                calldata = toolInput.mandateCalldata;
            }
            else {
                ...
                if (!template) {
                    return (`No encoding template registered for mandate ${mandateId} ` +
                        `(target: ${mandate.targetMandate}). Provide raw hex mandateCalldata instead.`);
                }
                try {
                    calldata = template.encode(toolInput.parameters ?? {});
                }
                catch (err) {
                    return `Template encoding error for ${template.name}: ${String(err)}`;
                }
            }
            console.log(`[tool] propose_action mandate=${mandateId} reasoning="${reasoning}" session=${session.sessionId}`);
            try {
                const { actionId, txHash } = await propose(session, targetOrg, mandateId, calldata, nonce, uri);
        ...
        case 'cast_vote': {
            const targetOrg = resolveOrg(session, org, toolInput.powersAddress);
            const actionId = BigInt(toolInput.actionId);
            const support = Number(toolInput.support);
            const reasoning = String(toolInput.reasoning ?? '');
            // Verify action is active
            const actionData = await getActionData(targetOrg.chainId, targetOrg.powersAddress, actionId);
            ...
            const voted = await hasVoted(targetOrg.chainId, targetOrg.powersAddress, actionId, session.userAddress);
            if (voted) {
                return `Error: agent has already voted on action ${actionId}.`;
            }
            const supportLabel = support === 1 ? 'FOR' : support === 0 ? 'AGAINST' : 'ABSTAIN';
            console.log(`[tool] cast_vote action=${actionId} support=${supportLabel} session=${session.sessionId}`);
            try {
                const { txHash } = await castVoteWithReason(session, targetOrg, actionId, support, reasoning);
        ...
        case 'execute_action': {
            ...
            const calldata = actionData.calldata;
            const uri = await getActionUri(targetOrg.chainId, targetOrg.powersAddress, actionId);
            console.log(`[tool] execute_action action=${actionId} reasoning="${reasoning}" session=${session.sessionId}`);
            try {
                const { txHash } = await request(session, targetOrg, actionData.mandateId, calldata, actionData.nonce, uri);
