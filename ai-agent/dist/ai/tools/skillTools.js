...
export async function dispatchSkill(skill, input) {
    const { handler, handlerConfig } = skill.definition;
    const allowedDomains = handlerConfig.allowedDomains ?? [];
    switch (handler) {
        case 'fetch_url': {
        ...
        case 'coingecko_price': {
            const ids = Array.isArray(input.coinIds) ? input.coinIds : [String(input.coinId ?? 'bitcoin')];
            const vs = Array.isArray(input.vsCurrencies) ? input.vsCurrencies : ['usd'];
        ...
        case 'chainlink_price': {
            const feedAddress = String(input.feedAddress ?? handlerConfig.feedAddress ?? '');
