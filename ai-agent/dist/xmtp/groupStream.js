...
export function startGroupStream(session, reason) {
        ...
        while (true) {
            try {
                ...
                for await (const message of stream) {
                    try {
                        console.log(`[groupStream] message received — sender=${message.senderInboxId} conv=${message.conversationId} session=${session.sessionId}`);
                        ...
                        if (!matchedOrg) {
                            console.log(`[groupStream] no matching org for chain=${parsed.chainId} powers=${parsed.powersAddress}`);
                            continue;
                        }
                        const text = typeof message.content === 'string' ? message.content : '';
                        if (!text.trim())
                            continue;
                        console.log(`[groupStream] triggering reason — org=${matchedOrg.powersAddress} text="${text.slice(0, 80)}${text.length > 80 ? '…' : ''}"`);
                        const groupReply = async (replyText) => {
                            const now = Date.now();
                            const last = session.lastReplyAt.get(message.conversationId) ?? 0;
                        ...
                        console.error(`[groupStream] message handling error (session ${session.sessionId}):`, err);
                    }
                }
            }
            catch (err) {
                console.error(`[groupStream] stream error for session ${session.sessionId}, restarting in 5s:`, err);
