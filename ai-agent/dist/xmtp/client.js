...
export async function createXmtpClient(session) {
    ...
    const encryptionKey = Buffer.from(config.xmtp.dbEncryptionKey.startsWith('0x')
        ? config.xmtp.dbEncryptionKey.slice(2)
        : config.xmtp.dbEncryptionKey, 'hex');
    const agent = await Agent.create(signer, {
        dbPath,
        dbEncryptionKey: encryptionKey,
        env: config.xmtp.env,
    });
    console.log(`[xmtp] client created for session ${session.sessionId} addr=${agent.address}`);
    return agent;
}
