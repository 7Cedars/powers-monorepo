...
export async function backfillGroupChatHistory(session) {
    ...
    await Promise.all(groups.map(async (group) => {
        const conversationId = group.id ?? group.conversationId;
        if (!conversationId)
            return;
        // Don't overwrite history from an already-active conversation.
        if (session.histories.has(conversationId))
            return;
        try {
            await group.sync();
            // direction: 1 = Descending (newest first), then reverse for chronological order.
            const messages = await group.messages({ limit: BACKFILL_LIMIT, direction: 1 });
            ...
            if (textLines.length === 0)
                return;
            const primingContent = `[CHAT HISTORY — last ${textLines.length} messages, loaded at startup]\n` +
                textLines.join('\n');
            session.histories.set(conversationId, [
                { role: 'user', content: primingContent },
                { role: 'assistant', content: '[Context loaded.]' },
            ]);
            console.log(`[chatHistory] backfilled ${textLines.length} messages for group ${conversationId} (session ${session.sessionId})`);
        }
        catch (err) {
            console.error(`[chatHistory] failed to backfill group ${conversationId} (session ${session.sessionId}):`, err);
        }
    }));
}
