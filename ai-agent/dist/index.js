...
async function onSessionStart(sessionId) {
    ...
    backfillGroupChatHistory(session).catch((err) => console.error(`[index] chat history backfill failed for ${sessionId}:`, err));
    // 2. Start group message stream — org is resolved from the group name inside the stream
    startGroupStream(session, async (sess, org, conversationId, triggerText, groupReply) => {
    ...
    for (const org of session.organisations) {
        requestOrgGroupAccess(session, org).catch((err) => console.error(`[index] requestOrgGroupAccess error for ${org.powersAddress}:`, err));
    }
    ...
    for (const org of session.organisations) {
        ...
        getActionHistory(org.chainId, org.powersAddress, 30)
            ...
            .catch((err) => console.error(`[index] action history load failed for ${org.powersAddress}:`, err));
    }
    // 7. Discover linked Powers instances for each org (fire-and-forget)
    for (const org of session.organisations) {
        discoverLinkedInstancesForOrg(session, org);
    }
    console.log(`[index] session ${sessionId} started — addr=${session.userAddress} ` +
        `orgs=${session.organisations.length}`);
}
function onSessionDestroy(sessionId) {
    const session = sessionManager.getSession(sessionId);
    if (session) {
        for (const org of session.organisations) {
...
function discoverLinkedInstancesForOrg(session, org) {
    ...
    getAllMandates(org.chainId, org.powersAddress)
        ...
        .catch((err) => console.error(`[index] linked instance discovery failed for ${org.powersAddress}:`, err));
}
// Called by the API server when a new org is added to a running session
function onOrgAdded(sessionId, org) {
    const session = sessionManager.getSession(sessionId);
    if (!session)
        return;
    startOrgListeners(session, org);
    requestOrgGroupAccess(session, org).catch((err) => console.error(`[index] requestOrgGroupAccess error for new org ${org.powersAddress}:`, err));
    discoverLinkedInstancesForOrg(session, org);
