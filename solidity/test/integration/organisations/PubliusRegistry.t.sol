...
contract PubliusRegistry_IntegrationTest is Test {
    ...
    function test_RegistryGovernanceFlow() public {
        // Fetch mandate IDs
        registerMandateId =
            findMandateIdInOrg("Register Mandate: Admin can register new mandates in the registry.", powers);
        deactivateMandateId =
            findMandateIdInOrg("Deactivate Mandate: Admin can deactivate mandates in the registry.", powers);
        reactivateMandateId =
            findMandateIdInOrg("Reactivate Mandate: Admin can reactivate mandates in the registry.", powers);
        ...
        // Get the real version to test against
        (uint16 major, uint16 minor, uint16 patch) = IMandate(realMandateAddress).version();

        bytes memory registerCalldata = abi.encode(mandateName, realMandateAddress);
        uint256 nonce = 1;
        ...
        // ----------------------------------------
        bytes memory deactivateCalldata = abi.encode(major, minor, patch, mandateName);
        nonce++;
        ...
        assertFalse(registry.isVersionActive(major, minor, patch, mandateName), "Mandate should be inactive");

        vm.stopPrank();

        // Reactivate Mandate Flow
        // ----------------------------------------
        bytes memory reactivateCalldata = abi.encode(major, minor, patch, mandateName);
        nonce++;
        ...
        assertTrue(registry.isVersionActive(major, minor, patch, mandateName), "Mandate should be active again");
