...
// import { ElectionRegistry_Tally } from "@src/mandates/integrations/ElectionRegistry_Tally.sol";
// import { PeerSelect } from "@src/mandates/electoral/PeerSelect.sol";
// import { ElectionRegistry_Vote } from "@src/mandates/integrations/ElectionRegistry_Vote.sol";
...
// ///      mandateId 1: ElectionRegistry_Tally (Erc20DelegateElection, roleId=3, maxHolders=3)
// ///      mandateId 2: PeerSelect (maxHolders=2, roleId=4, maxVotes=1, Nominees)
// ///      mandateId 3: ElectionRegistry_Vote (ElectionRegistry, maxVotes=1)
...
//     ElectionRegistry_Tally openElectionEnd;
//     PeerSelect peerSelect;
//     ElectionRegistry_Vote openElectionVote;
//     TaxSelect taxSelect;
...
//         openElectionEnd = ElectionRegistry_Tally(mandateAddresses[11]);
//         peerSelect = PeerSelect(mandateAddresses[12]);
//         openElectionVote = ElectionRegistry_Vote(mandateAddresses[13]);
...
//         openElection = ElectionRegistry(helperAddresses[9]);
...
//     /// @notice Fuzz test ElectionRegistry_Tally (mandateId 1) action generation
//     /// @dev mandateId 1 runs delegate elections for role 3
//     function testFuzzElectionRegistry_TallyActionGeneration(
...
//     /// @notice Fuzz test ElectionRegistry_Tally with various nonces
//     function testFuzzElectionRegistry_TallyWithVariousNonces(uint256 nonce1, uint256 nonce2) public {
...
//     /// @notice Fuzz test ElectionRegistry_Vote (mandateId 3) with various candidates
//     /// @dev mandateId 3 allows voting in open elections with max 1 vote == ElectionRegistry_Vote.
//     function testFuzzElectionRegistry_VoteWithCandidates(
...
//         // step 1: nominate candidates in ElectionRegistry contract
//         vm.startPrank(address(daoMock));
...
//         // step 2: initialise a NEW ElectionRegistry_Vote mandate
//         delete conditions;
...
//                 config: abi.encode(
//                     helperAddresses[9], // ElectionRegistry contract
//                     1 // max votes per voter
//                 ),
//                 conditions: conditions
//             })
//         );
//         // step 3: vote for candidates in ElectionRegistry_Vote contract
//         vm.prank(address(daoMock));
//         (returnedActionId, returnedTargets,, returnedCalldatas) =
...
//     function testFuzzElectionRegistry_VoteWithMultipleVotes(
//         address[] memory candidatesFuzzed,
//         uint256 quantity,
//         uint256 nonceFuzzed,
...
//         // step 2: initialise a NEW ElectionRegistry_Vote mandate with multiple votes allowed
//         delete conditions;
//         conditions.allowedRole = type(uint256).max;
//         mandateId = daoMock.mandateCounter();
//         vm.prank(address(daoMock));
...
//         // For ElectionRegistry_Vote, we need a bool array, not address array
...
//         // Test ElectionRegistry_Vote with no votes selected
...
//         // Nominate many candidates in ElectionRegistry
//         vm.startPrank(address(daoMock));
//         for (i = 0; i < numberOfNominees; i++) {
...
//         // Initialize a new ElectionRegistry_Vote mandate for this test
//         delete conditions;
//         conditions.allowedRole = type(uint256).max;
//         mandateId = daoMock.mandateCounter();
//         vm.prank(address(daoMock));
