...
| Role | Name & Description | Base contract | User Input | Executable Output | Conditions |
| :---- | :---- | :---- | :---- | :---- | :---- |
| Executives | Veto Call to sub-Dao | PowersAction\_Flexible.sol | "Address PowersTarget,uint16 MandateIdTarget,  uint16\[\] MandateId, uint256\[\] roleIds" | Calls to sub-DAOs | Executioners can veto calls to Powers instances in other sub-DAOs. |
...
The Ideas Sub-DAO can set its own role labels. 

| Role | Name & Description | Base contract | User Input | Executable Output | Conditions |
| :---- | :---- | :---- | :---- | :---- | :---- |
| Role 1 (Members) | Veto setting role labels | StatementOfIntent.sol | "uint256 RoleId, string Label" | None | Vote. |
| Role 2 (Conveners) | Set role labels | BespokeAction\_OnOwnPowers.sol | (same as above) | Sets role label | Vote. |

#### 

#### Mint activity token

A public mandate through which anyone can mint Activity Tokens. The mandate is throttled: only one token can be minted every five minutes.
...
Gives the Ideas Sub-DAO the power to incubate new initiatives. Conveners can propose the creation of internal Working Groups to focus on specific cultural projects that can subsequently spawn physical sub-DAO.

| Role | Name & Description | Base contract | User Input | Executable Output | Conditions |
| :---- | :---- | :---- | :---- | :---- | :---- |
| Conveners | Request new Physical Sub-DAO | StatementOfIntent.sol | "string name, string uri" | Requests mandate at Parent DAO | None |
| Conveners | Create new Working group | Mandates\_Prepackaged.sol | (same as above) | Installs Working Group Flow | Vote. Includes: Request new Physical Sub-DAO, Election for WG members. |
...
Membership is can be claimed by active participants who hold a specific number of Activity Tokens minted within a recent timeframe.
...
\-- BEGIN PROPOSAL FOR CHANGES \--    
Current issues: 

- The RWA framework only integrates with ERC-20 tokens (not NFTs or ERC-1155s). This makes it actually a very poor fit for our use case.   
- The RWA framework is very complex.   
- There is currently no distinction between the real world assets of the space being used (like props, panels, keys, etc) and physical items (art, tickets) being sold. As we have a payment flow based on selling items, making a distinction between use and sell items is important. 

Considerations 

- One of the key considerations: What really needs to be on-chain, what can be left off-chain?  
  - Any item that is being sold needs to be on-chain.   
  - Access control (as it is linked to the sub-DAO) needs to be managed on-chain.   
  - The space itself \+ props do (I think) NOT need to be on-chain.   
- It might be an idea to have ‘legal reps’ as a role in a physical DAO, and they are the legal point of reference for registered organisation  / foundation / company that represents the sub-DAO. Leasing the space, handling physical use items would fall on them.   
  - This leaves a lot of RWA issues off-chain. Avoiding issues of having to link on-chain and off-chain representation of the same item.   
  - Payment for these items can go through a desiccated payment flow initiated by the legal reps, and checked by the conveners.   
  - The legal reps will have the power to pause the physical DAO if any issues emerge.  

Proposed changes 

| Action | Flow | Comment |
| :---- | :---- | :---- |
| REMOVE | All governance flows to do with RWAs |  |
| ADD | Dedicated payment flow for Legal reps. Can be vetoed by conveners. | Used to pay for rent, etc |
| REMAIN | Payment of receipt flow (conveners)  | Used pay receipts to people who did work for the Physical DAO  |
| ADD | Flow to remove \-\> reinsert mandates (legal reps) | Control mechanism |
| ADD | Payment flow selling art | As in the other proposal.  |

What do you think of these changes? If you agree, I will implement. 

\-- END PROPOSAL FOR CHANGES \-- 
...
A framework for managing Real World Assets (RWAs) on-chain. Includes creating asset tokens, setting compliance rules, and managing transfers, ensuring digital tokens correctly reflect physical ownership and legal status.
