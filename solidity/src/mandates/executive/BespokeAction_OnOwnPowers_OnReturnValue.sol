...
contract BespokeAction_OnOwnPowers_OnReturnValue is Mandate {
    ...
    function initializeMandate(
        ...
        (,, string[] memory params_,,) = abi.decode(config, (bytes4, bytes, string[], uint16, bytes));
        ...
        returns (uint256 actionId, address[] memory targets, uint256[] memory values, bytes[] memory calldatas)
    {
        ...
        ) = abi.decode(getConfig(powers, mandateId), (bytes4, bytes, string[], uint16, bytes));
