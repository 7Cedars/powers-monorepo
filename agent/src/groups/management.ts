...
export async function addMembersToGroup(
  ...
  for (const address of addresses) {
    try {
      ...
      // Add member by address (Agent SDK handles inbox ID resolution)
      await group.addMembers([address.toLowerCase()]);
      successfulAdds.push(address);
    } catch (error) {
      console.error(`Failed to add ${address} to group:`, error);
      // Continue with other members
    }
  }
  
  console.log(`Successfully added ${successfulAdds.length} members to ${group.name}`);
}
...
export async function removeMembersFromGroup(
  ...
  for (const address of addresses) {
    try {
      // Remove member by address
      await group.removeMembers([address.toLowerCase()]);
      successfulRemovals.push(address);
    } catch (error) {
      console.error(`Failed to remove ${address} from group:`, error);
      // Continue with other members
    }
  }
  
  console.log(`Successfully removed ${successfulRemovals.length} members from ${group.name}`);
}
