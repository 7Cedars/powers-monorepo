      ...
      try {
        ...
        try {
          ...
          if (membersToAdd.length > 0) {
            ...
            // Filter to only members who can message on XMTP
            const validMembers: Address[] = [];
            for (const member of membersToAdd) {
              const canMessage = await tryToSendDM(agent, member, ''); // Empty message to check
              if (canMessage) {
            ...
            if (validMembers.length > 0) {
              // Add members using Agent SDK group methods
              // Note: This will need to be updated with correct Agent SDK API
              await (group as any).addMembersByAddress(validMembers);
              console.log(`Successfully added ${validMembers.length} members to ${chatroomId}`);
            } else {
              console.log(`No valid XMTP members found for group ${chatroomId}`);
            }
          } else {
        ...
        await (group as any).send(welcomeMessage);
