...
Structural changes beyond adopting mandates — revoking, relabelling roles, adding or repointing flows — are done by adopting a **configured `PresetActions_OnOwnPowers`** whose `bytes[]` config is a list of calls the organisation makes on itself. See `/design-org` Appendix A.2, "Governance Self-Modification". Do not propose `OpenAction` for this. Note that `PresetActions_OnOwnPowers` — unlike plain `PresetActions` — does **not** self-revoke; if the self-modification is meant to run only once, either include a `revokeMandate(itself)` call at the end of its own `bytes[]` config or revoke it in a follow-up step.
...
- **Bundles are all-or-nothing.** Any failed call reverts the whole reform. In particular, `revokeMandate` on an already-inactive mandate reverts — so a stale read can fail everything. Re-read state immediately before executing. **`PresetActions` self-revokes after it executes**, so any `PresetActions` mandate that has already run is already inactive: do not include it in a `revokeMandate` bundle, and never assume a `PresetActions` you saw adopted is still active. Confirm each target's `active` flag (`getAdoptedMandate`) at execution time before revoking it.
...
- **Re-adopting is not restoring.** A re-adopted mandate gets a *new* id, so conditions elsewhere that referenced the old id are not repaired by re-adoption. This is why `Pause_Mandates` repoints flows explicitly.
