...
This is a two step process to gain membership to the Primary DAO. 
1. **Request from Ideas Sub-DAO**: An Ideas Sub-DAO forwards a request for membership on behalf of a user.
2. **Gated Access Check**: A public user (can be anyone) calls the second step, which verifies if the prospective member holds **2 POAPs** (Proof of Attendance Protocols) from Physical Sub-DAOs that are **not older than 180 days** (approx. 6 months). If this check passes, the membership role is assigned.
