    ...
    <section id="examples" className="w-full flex flex-col items-center px-4 bg-muted/25 border-t border-border py-16">
      ...
      <div className="w-full flex flex-col justify-center items-center pb-10">
        ...
        <div className="w-full flex flex-col gap-4 justify-center items-center text-muted-foreground max-w-3xl text-center text-pretty font-mono">
          <span className="md:text-xl text-lg">Each organisation built on Powers Protocol comes with two interface components out of the box: a <span className="font-semibold text-foreground">forum</span>, where members chat, propose, and vote; and an <span className="font-semibold text-foreground">overview</span>, which displays the full history of every decision and transaction the organisation has made on-chain. These aren't locked-in products — they're functional starting points that any organisation can build on and customise to their own needs.</span>
          <span className="text-sm leading-relaxed">Below are some live implementations:</span>
        </div>
      </div>

      <div className="w-full max-w-3xl flex flex-col gap-0 border border-border bg-background">

        {/* Banner — 2:1 placeholder */}
        <div className="w-full aspect-[2/1] flex-shrink-0 bg-muted/30 border-b border-border flex items-center justify-center">
          <span className="font-mono text-xs text-muted-foreground uppercase tracking-wider">Banner image</span>
        </div>

        {/* Title */}
        <div className="px-6 pt-5 pb-2 font-mono font-bold text-foreground uppercase tracking-wider text-base border-t border-border text-center">
          {org.title}
        </div>

        {/* Description */}
        <div className="px-6 pb-5 font-mono text-sm text-muted-foreground leading-relaxed overflow-hidden text-center" style={{ height: '160px' }}>
          {org.description}
        </div>
      ...
      {/* Controls — same style as Applications section */}
      ...
      <span className="font-mono text-xs text-muted-foreground uppercase tracking-wider pt-3">
        {selected + 1} / {total}
      </span>
